...
export const AIChat: React.FC<AIChatProps> = ({ history, onSendMessage, isProcessing }) => {
    ...
    return (
        <div className="fixed right-4 top-36 z-50 flex flex-row-reverse items-start gap-3 pointer-events-none">
            {/* Floating Toggle Button */}
            <button
                onClick={() => setIsOpen(!isOpen)}
                className={`pointer-events-auto w-10 h-10 flex items-center justify-center rounded-xl shadow-md border transition-all duration-200 font-bold text-xs shrink-0 ${isOpen
                    ? 'bg-black text-white border-black'
                    : 'bg-white/90 backdrop-blur-sm text-black border-gray-200 hover:bg-gray-50 hover:border-gray-300'
                    }`}
                title="AI Chat"
            >
                {isOpen ? <X className="w-5 h-5" /> : "AI"}
            </button>

            {/* Chat Panel */}
            <div
                className={`pointer-events-auto w-80 sm:w-96 bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 overflow-hidden transition-all duration-300 origin-top-right flex flex-col ${isOpen
                    ? 'opacity-100 scale-100 translate-x-0 visible'
                    : 'opacity-0 scale-95 translate-x-8 invisible'
                    }`}
                style={{
                    // Dynamic height to fit within viewport, preventing overlap with zoom controls
                    height: isOpen ? 'calc(100vh - 250px)' : '0px',
                    maxHeight: '600px',
                    minHeight: '300px'
                }}
            >
                {/* Header with Model Selector */}
                <div className="p-3 border-b border-gray-100 bg-gray-50/80 flex items-center justify-between relative z-20">
                ...
                {/* Messages */}
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white/50 no-scrollbar">
                    {history.length === 0 && (
                    ...
                    {history.map((msg) => (
                        <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-gray-200 text-gray-600' : 'bg-black text-white'
                                }`}>
                ...
                <div className="bg-white border-t border-gray-100 p-2">
                    ...
                    <form onSubmit={handleSubmit} className="flex gap-2 items-end">
                        ...
                        <input
                            ...
                            className="flex-1 bg-gray-100 border-none rounded-xl px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:ring-2 focus:ring-black/5 outline-none mb-1"
                        />
                        <button
                            type="submit"
                            disabled={(!input.trim() && attachments.length === 0) || isProcessing}
                            className="p-3 bg-black text-white rounded-xl hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-colors mb-1"
                        >
